...
func TestProductRepository_Save_Err(t *testing.T) {
	...
	repo := NewMysqlProductRepository(transaction.NewTxHandler(db).SafeQuerier(), 1*time.Millisecond)
...
func TestProductRepository_Of_Success(t *testing.T) {
	...
	repo := NewMysqlProductRepository(transaction.NewTxHandler(db).SafeQuerier(), 60*time.Second)
